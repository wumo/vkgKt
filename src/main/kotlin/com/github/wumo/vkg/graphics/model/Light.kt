...
class Light(val scene: SceneManager, val id: Int) {
  var color: Vec3 = Vec3().also { LightGetColor(scene.native.notNull(), id, it.raw) }
    set(value) {
      LightSetColor(scene.native.notNull(), id, value.raw)
      field = value
    }
  var location: Vec3 = Vec3().also { LightGetLocation(scene.native.notNull(), id, it.raw) }
    set(value) {
      LightSetLocation(scene.native.notNull(), id, value.raw)
      field = value
    }
