...
class Renderer(internal val native: CRenderer, internal val closeFunc: ()->Unit): AutoCloseable {
  companion object {
    fun newBasicRenderer(windowConfig: WindowConfig): Renderer {
      ...
      val native = NewBasicRenderer(windowConfig_)
    ...
    fun newDeferredRenderer(windowConfig: WindowConfig, sceneConfig: DeferredSceneConfig): Renderer {
      val titleBytes = windowConfig.title.bytes()
      val windowConfig_ = CWindowConfig().also {
        it.title(titleBytes)
        it.width(windowConfig.width)
      ...
      val native = NewDeferredRenderer(windowConfig_, sceneConfig_)
      return Renderer(native) {
    ...
    fun newRayTracingRenderer(windowConfig: WindowConfig, sceneConfig: RayTracingSceneConfig): Renderer {
      val titleBytes = windowConfig.title.bytes()
      val windowConfig_ = CWindowConfig().also {
        it.title(titleBytes)
        it.width(windowConfig.width)
      ...
      val native = NewRayTracingRenderer(windowConfig_, sceneConfig_)
      return Renderer(native) {
    ...
    fun newPathTracingRenderer(windowConfig: WindowConfig, sceneConfig: PathTracingSceneConfig): Renderer {
      val titleBytes = windowConfig.title.bytes()
      val windowConfig_ = CWindowConfig().also {
        it.title(titleBytes)
        it.width(windowConfig.width)
      ...
      val native = NewPathTracingRenderer(windowConfig_, sceneConfig_)
      return Renderer(native) {
